...
FLValue make_literal(FLValue type, FLValue value, FLValue line);
...
FLValue make_block(FLValue type, FLValue name, FLValue fields, FLValue line);
...
FLValue make_pattern_list(FLValue items, FLValue rest, FLValue line);
...
FLValue make_template_string(FLValue parts, FLValue expressions, FLValue line);
static FLValue __fl_wrap_make_template_string(FLClosure*, int, FLValue*);
FLValue make_loop(FLValue init, FLValue condition, FLValue update, FLValue body, FLValue line);
...
FLValue cgc_defn_impl(FLValue args);
static FLValue __fl_wrap_cgc_defn_impl(FLClosure*, int, FLValue*);
...
FLValue cgc_join_hoisted();
static FLValue __fl_wrap_cgc_join_hoisted(FLClosure*, int, FLValue*);
...
static FLValue cgc_hoisted_fwds_atom;
...
static FLValue __fl_wrap_cgc_defn_impl(FLClosure* _s, int _ac, FLValue* argv) {
    (void)_s; (void)_ac;
    return cgc_defn_impl(argv[0]);
}
...
static FLValue __fl_wrap_cgc_join_hoisted(FLClosure* _s, int _ac, FLValue* argv) {
    (void)_s; (void)_ac;
    return cgc_join_hoisted();
}
...
FLValue is_alnum_p(FLValue c) {
    return fl_or(is_digit_p(c), is_alpha_p(c));
}
...
FLValue is_symbol_char_p(FLValue c) {
    return (fl_truthy(null_p(c)) ? fl_bool(false) : fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(fl_or(is_alnum_p(c), fl_eq(c, fl_str_val("-"))), fl_eq(c, fl_str_val("_"))), fl_eq(c, fl_str_val("?"))), fl_eq(c, fl_str_val("!"))), fl_eq(c, fl_str_val("/"))), fl_eq(c, fl_str_val("."))), fl_eq(c, fl_str_val("<"))), fl_eq(c, fl_str_val(">"))), fl_eq(c, fl_str_val("="))), fl_eq(c, fl_str_val("+"))), fl_eq(c, fl_str_val("*"))), fl_eq(c, fl_str_val("%"))), fl_eq(c, fl_str_val("&"))), fl_eq(c, fl_str_val("|"))), fl_eq(c, fl_str_val("^"))), fl_eq(c, fl_str_val("~"))));
}
...
FLValue read_number_body(FLValue st, FLValue acc, FLValue has_dot, FLValue line, FLValue col) {
    return read_number_iter(st, acc, has_dot, line, col);
}

FLValue read_number(FLValue st) {
    return read_number_body(st, fl_str_val(""), fl_bool(false), get(st, fl_str_val("line")), get(st, fl_str_val("col")));
}
...
FLValue read_string_body(FLValue st, FLValue acc, FLValue line, FLValue col) {
    return read_string_iter(st, acc, line, col);
}
...
FLValue read_symbol_body_kind(FLValue st, FLValue acc, FLValue line, FLValue col, FLValue kind) {
    return read_symbol_iter(st, acc, line, col, kind);
}

FLValue read_symbol(FLValue st) {
    return read_symbol_body_kind(st, fl_str_val(""), get(st, fl_str_val("line")), get(st, fl_str_val("col")), fl_str_val("Symbol"));
}
...
FLValue lex(FLValue src) {
    return lex_loop(make_state(src));
}
...
FLValue make_number(FLValue v, FLValue line) {
    return make_literal(fl_str_val("number"), v, line);
}

FLValue make_string(FLValue v, FLValue line) {
    return make_literal(fl_str_val("string"), v, line);
}

FLValue make_bool(FLValue v, FLValue line) {
    return make_literal(fl_str_val("boolean"), v, line);
}

FLValue make_null(FLValue line) {
    return make_literal(fl_str_val("null"), fl_nil(), line);
}

FLValue make_symbol(FLValue v, FLValue line) {
    return make_literal(fl_str_val("symbol"), v, line);
}
...
FLValue make_array_block(FLValue items, FLValue line) {
    return make_block(fl_str_val("Array"), fl_nil(), (__extension__ ({ FLValue __fl_kv[2] = {fl_str_val("items"), items}; fl_map_from_pairs(__fl_kv, 1); })), line);
}

FLValue make_map_block(FLValue items, FLValue line) {
    return make_block(fl_str_val("Map"), fl_nil(), (__extension__ ({ FLValue __fl_kv[2] = {fl_str_val("items"), items}; fl_map_from_pairs(__fl_kv, 1); })), line);
}
...
FLValue deep_equal_p(FLValue a, FLValue b) {
    return (fl_truthy(fl_and(null_p(a), null_p(b))) ? fl_bool(true) : (fl_truthy(fl_or(null_p(a), null_p(b))) ? fl_bool(false) : (fl_truthy(fl_and(list_p(a), list_p(b))) ? deep_equal_list_p(a, b, fl_int(0)) : (fl_truthy(fl_and(fl_map_p(a), fl_map_p(b))) ? deep_equal_map_p(a, b) : fl_eq(a, b)))));
}

FLValue deep_equal_list_p(FLValue a, FLValue b, FLValue i) {
    return (fl_truthy(fl_not(fl_eq(length(a), length(b)))) ? fl_bool(false) : (fl_truthy(fl_gte(i, length(a))) ? fl_bool(true) : (fl_truthy(fl_not(deep_equal_p(get(a, i), get(b, i)))) ? fl_bool(false) : deep_equal_list_p(a, b, fl_add(i, fl_int(1))))));
}
...
FLValue keys_no_line(FLValue m) {
    return fl_filter_fn(fl_fn_new(__fl_anon_0, 0, NULL), json_keys(m));
}
...
})))) ? fl_bool(false) : deep_equal_map_keys_p(a, b, ks, fl_add(i, fl_int(1)))));
}
...
FLValue p_append_ast(FLValue p, FLValue node) {
    return p_with_ast(p, fl_vec_push(get(p, fl_str_val("ast")), node));
}
...
FLValue parse_expr(FLValue p) {
    ...
    FLValue k = get(t, fl_str_val("kind"));
    FLValue v = get(t, fl_str_val("value"));
    (fl_truthy(fl_eq(k, fl_str_val("LParen"))) ? parse_sexpr(p) : (fl_truthy(fl_eq(k, fl_str_val("LBracket"))) ? parse_bracket(p) : (fl_truthy(fl_eq(k, fl_str_val("LBrace"))) ? parse_map(p) : (fl_truthy(fl_and(fl_eq(k, fl_str_val("Unknown")), fl_eq(v, fl_str_val("@")))) ? ((__extension__ ({
    FLValue p1 = p_advance(p);
    FLValue res = parse_expr(p1);
...
FLValue parse_sexpr(FLValue p) {
    ...
    FLValue p1 = p_advance(p);
    FLValue first = parse_args(p1, fl_vec_new());
    ...
    (fl_truthy(null_p(loop_array)) ? r_pair(parse_consume_rparen(p2), make_sexpr(fl_str_val("loop"), fl_vec_new(), line)) : (fl_truthy(fl_not(fl_eq(get(loop_array, fl_str_val("kind")), fl_str_val("array")))) ? r_pair(parse_consume_rparen(p2), make_sexpr(fl_str_val("loop"), rest, line)) : ((__extension__ ({
    ...
    r_pair(parse_consume_rparen(p2), make_loop(init, condition, update, (fl_truthy(fl_eq(length(body_exprs), fl_int(1))) ? get(body_exprs, fl_int(0)) : make_sexpr(fl_str_val("do"), body_exprs, line)), line));
})))));
}))) : (fl_truthy(fl_eq(op, fl_str_val("and"))) ? r_pair(parse_consume_rparen(p2), (__extension__ ({ FLValue __fl_kv[6] = {fl_str_val("kind"), fl_str_val("and"), fl_str_val("args"), rest, fl_str_val("line"), line}; fl_map_from_pairs(__fl_kv, 3); }))) : (fl_truthy(fl_eq(op, fl_str_val("or"))) ? r_pair(parse_consume_rparen(p2), (__extension__ ({ FLValue __fl_kv[6] = {fl_str_val("kind"), fl_str_val("or"), fl_str_val("args"), rest, fl_str_val("line"), line}; fl_map_from_pairs(__fl_kv, 3); }))) : r_pair(parse_consume_rparen(p2), make_sexpr(op, rest, line))))));
}))));
})));
}
...
FLValue parse_consume_rparen(FLValue p) {
    return ((__extension__ ({
    FLValue t = p_peek(p);
    (fl_truthy(fl_and(fl_not(null_p(t)), fl_eq(get(t, fl_str_val("kind")), fl_str_val("RParen")))) ? p_advance(p) : p);
})));
}
...
FLValue parse_bracket(FLValue p) {
    ...
    (fl_truthy(fl_and(fl_and(fl_and(fl_not(null_p(next)), fl_eq(get(next, fl_str_val("kind")), fl_str_val("Symbol"))), is_block_type_p(get(next, fl_str_val("value")))), fl_eq(get(next, fl_str_val("value")), upper_case(get(next, fl_str_val("value")))))) ? parse_named_block(p1, line) : parse_array(p1, line));
})));
}
...
FLValue parse_consume_rbracket(FLValue p) {
    return ((__extension__ ({
    FLValue t = p_peek(p);
    (fl_truthy(fl_and(fl_not(null_p(t)), fl_eq(get(t, fl_str_val("kind")), fl_str_val("RBracket")))) ? p_advance(p) : p);
})));
}
...
FLValue parse_named_block(FLValue p, FLValue line) {
    ...
    r_pair(parse_consume_rbracket(p3), make_block(type, name, fields, line));
})));
}
...
FLValue parse_optional_name(FLValue p) {
    return ((__extension__ ({
    FLValue t = p_peek(p);
    (fl_truthy(fl_and(fl_and(fl_not(null_p(t)), fl_eq(get(t, fl_str_val("kind")), fl_str_val("Symbol"))), fl_not(fl_eq(char_at(get(t, fl_str_val("value")), fl_int(0)), fl_str_val(":"))))) ? r_pair(p_advance(p), get(t, fl_str_val("value"))) : r_pair(p, fl_nil()));
})));
}

FLValue parse_block_fields(FLValue p, FLValue acc) {
    return (fl_truthy(p_end_p(p)) ? r_pair(p, acc) : ((__extension__ ({
    FLValue t = p_peek(p);
...
FLValue parse_consume_rbrace(FLValue p) {
    return ((__extension__ ({
    FLValue t = p_peek(p);
    (fl_truthy(fl_and(fl_not(null_p(t)), fl_eq(get(t, fl_str_val("kind")), fl_str_val("RBrace")))) ? p_advance(p) : p);
})));
}

FLValue parse_all(FLValue p) {
    return (fl_truthy(p_end_p(p)) ? get(p, fl_str_val("ast")) : ((__extension__ ({
    FLValue one = parse_expr(p);
...
FLValue parse(FLValue tokens) {
    return parse_all(p_make(tokens));
}
...
FLValue c_name(FLValue n) {
    ...
    (fl_truthy(c_reserved_p(raw)) ? fl_str_n(2, fl_str_val("fl_"), raw) : raw);
})));
}

FLValue cgc(FLValue n) {
    return (fl_truthy(null_p(n)) ? fl_str_val("fl_nil()") : (fl_truthy(fl_eq(get(n, fl_str_val("kind")), fl_str_val("literal"))) ? cgc_literal(n) : (fl_truthy(fl_eq(get(n, fl_str_val("kind")), fl_str_val("variable"))) ? ((__extension__ ({
    FLValue name = get(n, fl_str_val("name"));
    FLValue cn = c_name(name);
    FLValue line = get(n, fl_str_val("line"));
    (fl_truthy(fl_includes_item(fl_atom_deref(known_defns_atom), cn)) ? fl_str_n(3, fl_str_val("fl_fn_new(__fl_wrap_"), cn, fl_str_val(", 0, NULL)")) : (fl_truthy(fl_not(fl_includes_item(fl_atom_deref(known_fncall_targets_atom), cn))) ? (__extension__ ({ fl_println(fl_str_n(3, fl_str_val("[FL Warn] 정의되지 않은 이름: "), name, (fl_truthy(null_p(line)) ? fl_str_val("") : fl_str_n(3, fl_str_val(" (line "), line, fl_str_val(")"))))); cn;  })) : cn));
}))) : (fl_truthy(fl_eq(get(n, fl_str_val("kind")), fl_str_val("keyword"))) ? fl_str_n(3, fl_str_val("fl_str_val(\""), c_esc(get(n, fl_str_val("name"))), fl_str_val("\")")) : (fl_truthy(fl_eq(get(n, fl_str_val("kind")), fl_str_val("sexpr"))) ? cgc_sexpr(n) : (fl_truthy(fl_eq(get(n, fl_str_val("kind")), fl_str_val("block"))) ? cgc_block(n) : (fl_truthy(fl_eq(get(n, fl_str_val("kind")), fl_str_val("and"))) ? cgc_and(get(n, fl_str_val("args"))) : (fl_truthy(fl_eq(get(n, fl_str_val("kind")), fl_str_val("or"))) ? cgc_or(get(n, fl_str_val("args"))) : (__extension__ ({ fl_println(fl_str_n(4, fl_str_val("[CGC-ERR] unsupported IR kind="), get(n, fl_str_val("kind")), fl_str_val(" line="), get(n, fl_str_val("line")))); fl_str_val("fl_nil()");  }))))))))));
}
...
FLValue cgc_literal(FLValue n) {
    ...
    FLValue cn = c_name(v);
    (fl_truthy(fl_includes_item(fl_atom_deref(known_defns_atom), cn)) ? fl_str_n(3, fl_str_val("fl_fn_new(__fl_wrap_"), cn, fl_str_val(", 0, NULL)")) : cn);
...
FLValue cgc_params(FLValue it) {
    return cgc_params_loop(it, fl_int(0), fl_str_val(""));
}
...
FLValue cgc_params_loop(FLValue _it, FLValue _i, FLValue _acc) {
    ...
    FLValue n = cgc_extract_name(get(_it, i));
...
FLValue cgc_extract_name(FLValue node) {
    return (fl_truthy(fl_eq(get(node, fl_str_val("kind")), fl_str_val("variable"))) ? c_name(get(node, fl_str_val("name"))) : (fl_truthy(fl_eq(get(node, fl_str_val("kind")), fl_str_val("literal"))) ? c_name(fl_str_n(1, get(node, fl_str_val("value")))) : fl_str_val("_anon")));
}

FLValue cgc_fncall(FLValue fn_c, FLValue args) {
    return ((__extension__ ({
    FLValue argc = length(args);
    FLValue id = fl_atom_deref(lambda_id_atom);
...
FLValue cgc_dispatch_fallback(FLValue op, FLValue args) {
    return (fl_truthy(fl_eq(op, fl_str_val("first"))) ? fl_str_n(3, fl_str_val("fl_vec_first("), cgc(get(args, fl_int(0))), fl_str_val(")")) : (fl_truthy(fl_eq(op, fl_str_val("rest"))) ? fl_str_n(3, fl_str_val("fl_vec_rest("), cgc(get(args, fl_int(0))), fl_str_val(")")) : ((__extension__ ({
    FLValue cn = c_name(op);
    (fl_truthy(fl_includes_item(fl_atom_deref(known_fncall_targets_atom), cn)) ? cgc_fncall(cn, args) : fl_str_n(4, cn, fl_str_val("("), cgc_args(args), fl_str_val(")")));
})))));
}

FLValue cgc_fn_argv_decls(FLValue items, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(items))) ? acc : ((__extension__ ({
    FLValue name = c_name(cgc_extract_name(get(items, i)));
...
FLValue cgc_fn_param_names(FLValue items, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(items))) ? acc : cgc_fn_param_names(items, fl_add(i, fl_int(1)), fl_vec_push(acc, c_name(cgc_extract_name(get(items, i))))));
}
...
FLValue cgc_fn_caps_filter(FLValue all_vars, FLValue param_names, FLValue outer, FLValue i, FLValue acc) {
    ...
    cgc_fn_caps_filter(all_vars, param_names, outer, fl_add(i, fl_int(1)), (fl_truthy(fl_and(fl_not(fl_includes_item(param_names, v)), fl_includes_item(outer, v))) ? fl_vec_push(acc, v) : acc));
}))));
}
...
FLValue cgc_fn(FLValue args) {
    ...
    FLValue outer = fl_atom_deref(outer_params_atom);
...
FLValue cgc_str_arg(FLValue args) {
    return (fl_truthy(fl_eq(length(args), fl_int(1))) ? cgc(get(args, fl_int(0))) : cgc_str(args));
}
...
FLValue cgc_if(FLValue args) {
    ...
    FLValue fl_else = (fl_truthy(fl_gte(length(args), fl_int(3))) ? cgc(get(args, fl_int(2))) : fl_str_val("fl_nil()"));
...
FLValue cgc_cond(FLValue args) {
    ...
    (fl_truthy(nested) ? cgc_cond_nested(args, fl_sub(length(args), fl_int(1)), fl_str_val("fl_nil()")) : fl_str_val("fl_nil()"));
}))));
}
...
FLValue cgc_do(FLValue args) {
    return fl_str_n(3, fl_str_val("(__extension__ ({ "), cgc_stmts(args, fl_int(0), fl_str_val("")), fl_str_val(" }))"));
}
...
FLValue cgc_let_1d(FLValue it, FLValue i, FLValue acc) {
    ...
    FLValue n = cgc_extract_name(get(it, i));
...
FLValue cgc_let_2d(FLValue it, FLValue i, FLValue acc) {
    ...
    FLValue n = cgc_extract_name(get(p, fl_int(0)));
...
FLValue cgc_body(FLValue args, FLValue i, FLValue acc) {
    ...
    (fl_truthy(last) ? fl_str_n(2, acc, c) : cgc_body(args, fl_add(i, fl_int(1)), fl_str_n(3, acc, c, fl_str_val(";\n    "))));
}))));
}
...
FLValue cgc_defn_impl(FLValue args) {
    ...
    FLValue body = (fl_truthy(fl_eq(length(bodies), fl_int(1))) ? cgc(get(bodies, fl_int(0))) : cgc_do(bodies));
    fl_str_n(7, fl_str_val("FLValue "), name, fl_str_val("("), ps, fl_str_val(") {\n    return "), body, fl_str_val(";\n}"));
})));
})));
})));
...
FLValue cgc_defn(FLValue args) {
    return ((__extension__ ({
    FLValue depth = fl_atom_deref(cgc_defn_depth_atom);
    FLValue saved_params = fl_atom_deref(outer_params_atom);
    fl_atom_reset(cgc_defn_depth_atom, fl_add(fl_atom_deref(cgc_defn_depth_atom), fl_int(1)));
    ((__extension__ ({
    FLValue result = cgc_defn_impl(args);
    fl_atom_reset(cgc_defn_depth_atom, fl_sub(fl_atom_deref(cgc_defn_depth_atom), fl_int(1)));
    fl_atom_reset(outer_params_atom, saved_params);
    (fl_truthy(fl_gt(depth, fl_int(0))) ? ((__extension__ ({
    FLValue name = c_name(cgc_extract_name(get(args, fl_int(0))));
    FLValue param_items = get_block_items(get(args, fl_int(1)));
    FLValue ps = cgc_params(param_items);
    fl_atom_reset(cgc_hoisted_fns_atom, fl_vec_push(fl_atom_deref(cgc_hoisted_fns_atom), result));
    fl_atom_reset(known_defns_atom, fl_vec_push(fl_atom_deref(known_defns_atom), name));
    fl_atom_reset(cgc_hoisted_fwds_atom, fl_str_n(9, fl_atom_deref(cgc_hoisted_fwds_atom), fl_str_val("FLValue "), name, fl_str_val("("), ps, fl_str_val(");\n"), fl_str_val("static FLValue __fl_wrap_"), name, fl_str_val("(FLClosure*, int, FLValue*);\n")));
    fl_str_val("fl_nil()");
}))) : result);
})));
})));
...
FLValue cgc_binop_chain(FLValue args, FLValue fn) {
    return (fl_truthy(fl_eq(length(args), fl_int(0))) ? fl_str_val("fl_int(0)") : (fl_truthy(fl_eq(length(args), fl_int(1))) ? cgc(get(args, fl_int(0))) : cgc_binop_fold(args, fn, fl_int(1), cgc(get(args, fl_int(0))))));
}

FLValue cgc_binop_fold(FLValue args, FLValue fn, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(args))) ? acc : cgc_binop_fold(args, fn, fl_add(i, fl_int(1)), fl_str_n(6, fn, fl_str_val("("), acc, fl_str_val(", "), cgc(get(args, i)), fl_str_val(")"))));
}

FLValue cgc_and(FLValue args) {
    return (fl_truthy(fl_eq(length(args), fl_int(0))) ? fl_str_val("fl_bool(true)") : (fl_truthy(fl_eq(length(args), fl_int(1))) ? cgc(get(args, fl_int(0))) : cgc_and_fold(args, fl_int(1), cgc(get(args, fl_int(0))))));
}

FLValue cgc_and_fold(FLValue args, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(args))) ? acc : cgc_and_fold(args, fl_add(i, fl_int(1)), fl_str_n(5, fl_str_val("fl_and("), acc, fl_str_val(", "), cgc(get(args, i)), fl_str_val(")"))));
}

FLValue cgc_or(FLValue args) {
    return (fl_truthy(fl_eq(length(args), fl_int(0))) ? fl_str_val("fl_bool(false)") : (fl_truthy(fl_eq(length(args), fl_int(1))) ? cgc(get(args, fl_int(0))) : cgc_or_fold(args, fl_int(1), cgc(get(args, fl_int(0))))));
}

FLValue cgc_or_fold(FLValue args, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(args))) ? acc : cgc_or_fold(args, fl_add(i, fl_int(1)), fl_str_n(5, fl_str_val("fl_or("), acc, fl_str_val(", "), cgc(get(args, i)), fl_str_val(")"))));
}

FLValue cgc_args(FLValue args) {
    return cgc_args_loop(args, fl_int(0), fl_str_val(""));
}
...
FLValue cgc_forward_decls(FLValue nodes) {
    return cgc_forward_loop(nodes, fl_int(0), fl_str_val(""));
}
...
FLValue cgc_forward_loop(FLValue _nodes, FLValue _i, FLValue _acc) {
    ...
    FLValue ps = cgc_params(get_block_items(get(get(n, fl_str_val("args")), fl_int(1))));
    fl_atom_reset(known_defns_atom, fl_vec_push(fl_atom_deref(known_defns_atom), name));
...
FLValue cgc_top_level(FLValue _nodes, FLValue _i, FLValue _stmts, FLValue _fns) {
    ...
    FLValue _fl_t0 = fl_add(i, fl_int(1));
    FLValue _fl_t1 = fl_str_n(4, stmts, fl_str_val("    "), cgc(n), fl_str_val(";\n"));
    FLValue _fl_t2 = fns;
    i = _fl_t0;
    stmts = _fl_t1;
    fns = _fl_t2;
...
FLValue cgc_lambda_fwds() {
    return cgc_lambda_fwd_loop(fl_atom_deref(lambda_defs_atom), fl_int(0), fl_str_val(""));
}
...
FLValue cgc_join_lambdas() {
    return cgc_join_lambda_loop(fl_atom_deref(lambda_defs_atom), fl_int(0), fl_str_val(""));
}

FLValue cgc_join_wrappers() {
    return cgc_join_lambda_loop(fl_atom_deref(wrapper_defs_atom), fl_int(0), fl_str_val(""));
}

FLValue cgc_join_globals() {
    return cgc_join_lambda_loop(fl_atom_deref(global_decls_atom), fl_int(0), fl_str_val(""));
}

FLValue cgc_join_hoisted() {
    return cgc_join_lambda_loop(fl_atom_deref(cgc_hoisted_fns_atom), fl_int(0), fl_str_val(""));
}
...
FLValue generate_c(FLValue nodes) {
    ...
    FLValue hfwd = fl_atom_deref(cgc_hoisted_fwds_atom);
    FLValue hfns = cgc_join_hoisted();
...
FLValue cgc_set_b(FLValue args) {
    return fl_str_n(5, fl_str_val("(__extension__ ({ "), cgc(get(args, fl_int(0))), fl_str_val(" = "), cgc(get(args, fl_int(1))), fl_str_val("; fl_nil(); }))"));
}

FLValue cgc_while(FLValue args) {
    return fl_str_n(5, fl_str_val("(__extension__ ({ while (fl_truthy("), cgc(get(args, fl_int(0))), fl_str_val(")) { "), cgc(get(args, fl_int(1))), fl_str_val("; } fl_nil(); }))"));
}

FLValue loop_extract_vars(FLValue items, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(items))) ? acc : loop_extract_vars(items, fl_add(i, fl_int(2)), fl_vec_push(acc, cgc_extract_name(get(items, i)))));
}
...
FLValue cgc_recur_temps(FLValue args, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(args))) ? acc : cgc_recur_temps(args, fl_add(i, fl_int(1)), fl_str_n(6, acc, fl_str_val("    FLValue _fl_t"), i, fl_str_val(" = "), cgc(get(args, i)), fl_str_val(";\n"))));
}

FLValue cgc_recur_assigns(FLValue vars, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(vars))) ? acc : cgc_recur_assigns(vars, fl_add(i, fl_int(1)), fl_str_n(6, acc, fl_str_val("    "), get(vars, i), fl_str_val(" = _fl_t"), i, fl_str_val(";\n"))));
}
...
FLValue cgc_with_recur(FLValue node, FLValue vars) {
    ...
    (fl_truthy(fl_eq(op, fl_str_val("recur"))) ? cgc_recur_stmt(args, vars) : (fl_truthy(fl_eq(op, fl_str_val("if"))) ? cgc_if_wr(args, vars) : (fl_truthy(fl_eq(op, fl_str_val("cond"))) ? cgc_cond_wr(args, vars) : (fl_truthy(fl_eq(op, fl_str_val("do"))) ? cgc_do_wr(args, vars) : (fl_truthy(fl_eq(op, fl_str_val("let"))) ? cgc_let_wr(args, vars) : (fl_truthy(fl_eq(op, fl_str_val("loop"))) ? cgc_loop(args) : cgc(node)))))));
}))) : cgc(node));
}))));
}
...
FLValue cgc_if_wr(FLValue args, FLValue vars) {
    ...
    FLValue fl_else = (fl_truthy(fl_gte(length(args), fl_int(3))) ? cgc_with_recur(get(args, fl_int(2)), vars) : fl_str_val("fl_nil()"));
    fl_str_n(7, fl_str_val("(fl_truthy("), cond, fl_str_val(") ? "), then, fl_str_val(" : "), fl_else, fl_str_val(")"));
})));
}
...
FLValue cgc_do_wr(FLValue args, FLValue vars) {
    return fl_str_n(3, fl_str_val("(__extension__ ({ "), cgc_stmts_wr(args, vars, fl_int(0), fl_str_val("")), fl_str_val(" }))"));
}

FLValue cgc_stmts_wr(FLValue args, FLValue vars, FLValue i, FLValue acc) {
    return (fl_truthy(fl_gte(i, length(args))) ? acc : cgc_stmts_wr(args, vars, fl_add(i, fl_int(1)), fl_str_n(3, acc, cgc_with_recur(get(args, i), vars), fl_str_val("; "))));
}
...
FLValue cgc_body_wr(FLValue args, FLValue vars, FLValue i, FLValue acc) {
    ...
    (fl_truthy(last) ? c : cgc_body_wr(args, vars, fl_add(i, fl_int(1)), fl_str_n(3, acc, c, fl_str_val(";\n    "))));
}))));
}
...
FLValue cgc_map_entry_c(FLValue ent) {
    return fl_str_n(4, fl_str_val("fl_str_val(\""), c_esc(get(ent, fl_int(0))), fl_str_val("\"), "), cgc(get(ent, fl_int(1))));
}
...
FLValue cgc_map_key_c(FLValue key_node) {
    return (fl_truthy(fl_eq(get(key_node, fl_str_val("kind")), fl_str_val("keyword"))) ? fl_str_n(3, fl_str_val("fl_str_val(\""), c_esc(get(key_node, fl_str_val("name"))), fl_str_val("\")")) : cgc(key_node));
}
...
FLValue cgc_map_block(FLValue n) {
    ...
    (fl_truthy(fl_and(fl_not(null_p(items)), fl_eq(type_of(items), fl_str_val("array")))) ? cgc_map_from_items(items) : ((__extension__ ({
...
FLValue ir_err(FLValue msg, FLValue n) {
    return fl_println(fl_str_n(6, fl_str_val("[IR-ERR] "), msg, fl_str_val(" | kind="), get(n, fl_str_val("kind")), fl_str_val(" line="), get(n, fl_str_val("line"))));
}
...
FLValue ir_chk(FLValue n) {
    ...
    FLValue t = get(n, fl_str_val("type"));
    FLValue v = get(n, fl_str_val("value"));
    (fl_truthy(null_p(t)) ? ir_err(fl_str_val("literal missing :type"), n) : (fl_truthy(fl_not(fl_includes_item(ir_lit_types, t))) ? ir_err(fl_str_n(2, fl_str_val("literal bad type="), t), n) : (fl_truthy(fl_and(fl_eq(t, fl_str_val("boolean")), fl_not(fl_eq(type_of(v), fl_str_val("boolean"))))) ? ir_err(fl_str_n(2, fl_str_val("boolean value must be FL boolean, got "), type_of(v)), n) : (fl_truthy(fl_and(fl_eq(t, fl_str_val("nil")), fl_not(null_p(v)))) ? ir_err(fl_str_val("nil literal value must be nil"), n) : fl_bool(true)))));
...
int main(int argc, char** argv) {
    ...
    cgc_hoisted_fns_atom = fl_atom_new(fl_vec_new());;
    cgc_hoisted_fwds_atom = fl_atom_new(fl_str_val(""));;
    ir_kind_set = (__extension__ ({ FLValue __fl_arr[7] = {fl_str_val("literal"), fl_str_val("variable"), fl_str_val("keyword"), fl_str_val("sexpr"), fl_str_val("block"), fl_str_val("and"), fl_str_val("or")}; fl_vec_from(__fl_arr, 7); }));;
